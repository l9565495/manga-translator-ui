...
- **忽略非气泡文本 (ignore_bubble)**：智能过滤非对话框区域的文本
  ...
  - **工作原理**：
    ...
    - 正常白色气泡：边缘几乎全白（比例 < 阈值）→ 保留翻译
    - 正常黑色气泡：边缘几乎全黑（比例 > 100-阈值）→ 保留翻译
    - 非气泡区域：边缘黑白混杂（阈值 ≤ 比例 ≤ 100-阈值）→ 跳过
    - 彩色文字：检测到彩色 → 跳过
  - **阈值效果**：
    - **值越低**（如 5）：过滤范围 5-95，过滤更严格，可能误过滤正常气泡
    - **值越高**（如 20）：过滤范围 20-80，过滤更宽容，只过滤明显的非气泡区域
  - **使用场景**：
